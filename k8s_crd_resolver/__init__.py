...
def remove_k8s_descriptions(
    schema: Dict[str, Any], source_schema: Dict[str, Any]
) -> None:
    if isinstance(schema, dict):
        for k in list(schema.keys()):
...
def parse_and_resolve(
    schema: Dict[str, Any], *, remove_desciptions: bool = False
) -> Dict[str, Any]:
...
def resolve(source, destination, jsonpatch=None, remove_descriptions=False):
    ...
    if source["apiVersion"] == "apiextensions.k8s.io/v1beta1":
        resolved_schema = parse_and_resolve(
            source["spec"]["validation"]["openAPIV3Schema"],
